...
__device__ glm::vec3 computeVelocityChange(int N, int iSelf, const glm::vec3 *pos, const glm::vec3 *vel) {
    ...
    glm::vec3 pc(0.f, 0.f, 0.f);
    int numNeighborsR1 = 0;

    glm::vec3 c(0.f, 0.f, 0.f);

    glm::vec3 pv(0.f, 0.f, 0.f);
    int numNeighborsR3 = 0;

    // Check against every other boid
    for (int b = 0; b < N; ++b) {
        if (b == iSelf) {
            continue;
        }
...
__global__ void kernUpdateVelocityBruteForce(int N, glm::vec3 *pos,
  ...
  // Record the new velocity into vel2. Question: why NOT vel1?
    // I believe all threads need to read vel1 to compute the new velocity.
    // vel1 contains all "baseline" velocities that each thread equally depends on
    // recording the new velocity back into vel1 will interfere with thread ordering...
    // current index:
    int index = threadIdx.x + (blockIdx.x * blockDim.x);
    if (index >= N) {
        return;
    ...
    glm::vec3 newVel = vel1[index] + delta;
...
__global__ void kernComputeIndices(int N, int gridResolution,
  ...
  glm::vec3 *pos, int *indices, int *gridIndices) {
    ...
    int x = (int)floorf(cellPos.x);
    int y = (int)floorf(cellPos.y);
    int z = (int)floorf(cellPos.z);
...
__global__ void kernIdentifyCellStartEnd(int N, int *particleGridIndices,
  ...
  // "this index doesn't match the one before it, must be a new cell!"
    ...
    // Case 1: very first index (automatically start)
    // Case 2: the left index is not equal to the current cell (must be start of new cell)
    if (index == 0 || particleGridIndices[index - 1] != cell) {
        gridCellStartIndices[cell] = index;
    }
    // Case 1: very last index (automatically end)
    // Case 2: the right index is not equal to the current cell (must be end of the current cell)
...
__global__ void kernReshuffleParticleData(
    ...
    // this helper puts position and velocity data into the new coherent buffers
...
__global__ void kernUpdateVelNeighborSearchScattered(
  ...
  // - Clamp the speed change before putting the new speed in vel2
    ...
    // find current particle's grid cell
    glm::vec3 cellPos = (thisPos - gridMin) * inverseCellWidth;
    int cellX = (int)floorf(cellPos.x);
...
void Boids::stepSimulationNaive(float dt) {
  // 1.2 - use the kernels you wrote to step the simulation forward in time.
    ...
    checkCUDAErrorWithLine("kernUpdateVelocityBruteForce issue :(");

    kernUpdatePos << <fullBlocksPerGrid, blockSize >> > (numObjects, dt, dev_pos, dev_vel2);
    checkCUDAErrorWithLine("kernUpdatePos issue :(");
...
void Boids::stepSimulationScatteredGrid(float dt) {
  ...
  //   cell's data pointers in the array of boid indices
    kernResetIntBuffer << <fullBlocksPerGridCell, blockSize >> > (
...
void Boids::stepSimulationCoherentGrid(float dt) {
  ...
  //   CONSIDER WHAT ADDITIONAL BUFFERS YOU NEED
    kernReshuffleParticleData << <fullBlocksPerGrid, blockSize >> > (
        numObjects,
        dev_particleArrayIndices,
        dev_pos,
        dev_vel1,
